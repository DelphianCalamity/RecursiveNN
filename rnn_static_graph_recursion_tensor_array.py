...
class RecursiveNetStaticGraph():
    ...
    def __init__(self, config):
        ...
        rec = function.Declare("Rec", [("i", tf.int32), ("is_leaf", tf.int32), 
            ("node_word_indices", tf.int32), ("embeddings", tf.float32), ("W", tf.float32), ("b", tf.float32), ("tensor_array", tf.float32)], 
            [("ret", tf.float32)])
        ...
        def RecImpl(i, is_leaf, node_word_indices, embeddings, W, b, tensor_array):
            ...
            node_tensor = \
                tf.cond(tf.equal(tf.gather(is_leaf, i), tf.constant(1)),
                        ...
                        lambda: combine_children(rec(i*2, is_leaf, node_word_indices, embeddings, W, b, tensor_array),
                                               rec(i*2+1, is_leaf, node_word_indices, embeddings, W, b, tensor_array), W, b))
        ...
        self.node_tensor = rec(self.cons_placeholder, self.is_leaf_placeholder, 
                            self.node_word_indices_placeholder, self.embeddings, W1, b1, tensor_array)
